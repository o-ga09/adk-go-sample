...
7. **Go blog の要約・翻訳を試す**: Slack の bot に `@bot https://go.dev/blog/slices を要約して` のように話しかける。追加の環境変数は不要(読み取り専用ツール)。API 直叩きでも同様に呼び出せる:
   ...
   ```
8. **予定の確認・朝のダイジェスト(#14)**: Slack の bot に `@bot 今日の予定は？` のように話しかけると、`calendar_list_events`(読み取り専用、`ACTION_MODE` に関わらず常に有効)で当日の予定を取得しスレッドに返す。予定ゼロ件でも「本日の予定はありません」と返る。
   毎朝の一覧通知は `cmd/batch` を `-command calendar-digest` で実行する(既定は `-command mail`):
   ```sh
   go run ./cmd/batch -command calendar-digest
   ```
   バッチ起点はトップレベル(`SLACK_CHANNEL_ID`)投稿、Slack起点はスレッド返信という、既存の通知の振り分け(`internal/tools/notify`)がそのまま使われる。本番では `-command mail` とは別の ArgoWorkflows CronWorkflow ステップとして相乗りさせる想定(インフラリポ側の対応が必要)。
