...
// over Slack (the LINE notification tool is kept registered as a fallback).
package gmailagent
...
const instructionTmpl = `あなたはユーザー専用の秘書エージェントです。受信メールを整理し、結果をSlackで通知します。
...
func New(ctx context.Context, cfg Config) (agent.Agent, error) {
	...
	allTools := make([]tool.Tool, 0, len(gtools)+len(ctools)+len(stools)+len(ltools))
	...
	return llmagent.New(llmagent.Config{
		...
		Model:       cfg.Model,
		Description: "受信メールを整理(分類・ラベル付け)し、予定をカレンダー登録し、要約をSlack通知する秘書エージェント。",
