...
// either by URL or by listing recent posts when no URL is given (typically
...
// between three tasks based on what the message actually asks for.
...
- ツールがエラーを返したら、業務Aでは slack_push の note 引数に、業務B・Cでは返信にその旨を含める。無限ループを避け、各メール・各記事の処理は一度だけにする。
- いずれの業務にも当てはまらない依頼の場合は、対応できない旨を日本語で伝える。`
...
func New(ctx context.Context, cfg Config) (agent.Agent, error) {
	...
	return llmagent.New(llmagent.Config{
		...
		Model:       cfg.Model,
		Description: "受信メールを整理(分類・ラベル付け)し、予定をカレンダー登録し、要約をSlack通知する秘書エージェント。Go blogのURLを渡すと要約・翻訳も行い、URLがなくても最近の記事一覧・要約に対応する。",
