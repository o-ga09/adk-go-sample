...
// the user over Slack (the LINE notification tool is kept registered as a
// fallback). It also fetches, summarizes, and translates Go blog posts when
// asked to (typically via the Slack @mention listener).
package gmailagent
...
5. 最後に slack_push で日本語の要約を1通だけ送る。形式の例:
   「📬 メール整理完了
   ・要確認: N件 (件名を箇条書き)
   ・不要(ラベル付与): M件
   ・カレンダー登録: K件 (予定名と日時)」
...
- ツールがエラーを返したら、その旨を要約/返信に含める。無限ループを避け、各メール・各記事の処理は一度だけにする。
...
func New(ctx context.Context, cfg Config) (agent.Agent, error) {
	...
	ltools, err := notifytools.LineTools(cfg.App)
	if err != nil {
		return nil, fmt.Errorf("line notify tools: %w", err)
	}
	...
	allTools := make([]tool.Tool, 0, len(gtools)+len(ctools)+len(stools)+len(ltools)+len(btools))
	allTools = append(allTools, gtools...)
	allTools = append(allTools, ctools...)
	allTools = append(allTools, stools...)
	allTools = append(allTools, ltools...)
