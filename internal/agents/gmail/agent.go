...
// userMemory is the Git-managed long-term memory the agent carries across every
// session (#12): the owner's profile and standing preferences for mail
// classification / calendar / tasks. It is a plain Markdown file edited via
// PR — not a runtime-writable store — and is rendered into the instruction at
// build time via the {{.UserMemory}} field below, so batch/Slack/API
// invocations all see the same, reviewable set of user facts.
//
//go:embed memory.md
var userMemory string

// instructionTmpl is a text/template rendered once at build time from
// instructionData ({{.GmailQuery}} / {{.ActionMode}} / {{.UserMemory}}). Using
// text/template rather than fmt.Sprintf keeps the placeholders self-describing
// and sidesteps '%' escaping in the query or memory text. Note ADK's own
// single-brace {state} substitution is a distinct mechanism and is not used
// here.
//
...
1. gmail_list_messages を query="{{.GmailQuery}}" で呼び、対象メールの一覧を取得する。
...
# 注意
- 業務A・業務Dの現在の動作モード: {{.ActionMode}}。dry_run の場合、ツールは実際の変更を行わずログのみ返すが、手順は同じように実行すること。
...
// instruction is the parsed form of instructionTmpl. Parsing at package init
// (template.Must) fails fast on a malformed template rather than at first run.
var instruction = template.Must(template.New("instruction").Parse(instructionTmpl))

// instructionData fills the instruction template's placeholders.
...
func New(ctx context.Context, cfg Config) (agent.Agent, error) {
	...
	var instrBuf strings.Builder
	if err := instruction.Execute(&instrBuf, instructionData{
		GmailQuery: cfg.App.GmailQuery,
		ActionMode: string(mode),
		UserMemory: strings.TrimSpace(userMemory),
	}); err != nil {
		return nil, fmt.Errorf("render instruction: %w", err)
	}
